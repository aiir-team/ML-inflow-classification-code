...
def my_classify_metrics(y_true, y_pred, y_pred_prob=None, prefix="train", decimal=4):
    evaluator = ClassificationMetric(y_true, y_pred, decimal=decimal)
    paras = [{"average": "weighted"}, ] * len(Config.METRICS_FOR_TESTING_PHASE)
    mm2 = evaluator.get_metrics_by_list_names(Config.METRICS_FOR_TESTING_PHASE, paras)
    if y_pred_prob is not None:
        ...
        mm2["ROC_AUC"] = round(roc_auc_score(y_true, y_pred_prob, multi_class="ovr"), decimal)
    mm = {}
