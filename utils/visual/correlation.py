...
def draw_correlation(list_data: list, dict_metrics: dict, list_colors: list, xy_labels: list,
    ...
    if dict_metrics is not None:
        space = (np.max(list_data[1]) - np.min(list_data[1])) / 6
        for idx, (key, value) in enumerate(dict_metrics.items()):
            plt.text(np.min(list_data[0]), np.max(list_data[1])-0.5-0.35*idx*space, f'{key} = {value:.3f}',
...
#
# x = np.random.choice(list(range(0, 100)), 100, replace=False)
# y = np.random.choice(list(range(0, 5000)), 100, replace=False)
# print(x)
# print(y)
#
# list_data = [x, y]
